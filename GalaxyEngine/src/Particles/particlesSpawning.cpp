...
void ParticlesSpawning3D::particlesInitialConditions(Physics& physics, UpdateVariables& myVar, UpdateParameters& myParam) {
	...
	/*	if (myVar.isDragging && enablePathPrediction && myVar.gridExists) {
		...
		if ((IO::mouseDown(2) || (IO::mouseDown(0) && myVar.toolDrawParticles)) && !IO::shortcutDown(KEY_LEFT_CONTROL) && !IO::shortcutDown(KEY_LEFT_ALT) && !IO::shortcutDown(KEY_X)) {
			...
			myParam.brush3D.brushLogic(myParam, myVar.isSPHEnabled, myVar.constraintAfterDrawing, massScatter);
