...
void ParticleSelection::clusterSelection(UpdateVariables& myVar, UpdateParameters& myParam, bool externalTrigger) {
	...
	if ((IsMouseButtonPressed(2) && IsKeyDown(KEY_LEFT_CONTROL) && myVar.isMouseNotHoveringUI) || externalTrigger) {
		dragStartPos = glm::vec2(GetMousePosition().x, GetMousePosition().y);
		isMouseMoving = false;
	}

	if ((IsMouseButtonDown(2) && IsKeyDown(KEY_LEFT_CONTROL) && myVar.isMouseNotHoveringUI) || externalTrigger) {
	...
	if ((IsMouseButtonReleased(2) && IsKeyDown(KEY_LEFT_CONTROL) && !isMouseMoving && myVar.isMouseNotHoveringUI) || externalTrigger) {
...
void ParticleSelection::particleSelection(UpdateVariables& myVar, UpdateParameters& myParam, bool externalTrigger) {
	...
	if ((IsMouseButtonPressed(2) && IsKeyDown(KEY_LEFT_ALT) && myVar.isMouseNotHoveringUI) || externalTrigger) {
		dragStartPos = glm::vec2(GetMousePosition().x, GetMousePosition().y);
		isMouseMoving = false;
	}

	if ((IsMouseButtonDown(2) && IsKeyDown(KEY_LEFT_ALT) && myVar.isMouseNotHoveringUI) || externalTrigger) {
		glm::vec2 currentPos = glm::vec2(GetMousePosition().x, GetMousePosition().y);
		float dragThreshold = 5.0f;
		float dx = currentPos.x - dragStartPos.x;
		float dy = currentPos.y - dragStartPos.y;
	...
	if ((IsMouseButtonReleased(2) && IsKeyDown(KEY_LEFT_ALT) && !isMouseMoving && myVar.isMouseNotHoveringUI) || externalTrigger) {
...
void ParticleSelection::boxSelection(UpdateParameters& myParam, bool& is3DMode) {
	...
	if (IsMouseButtonDown(2) && !IsKeyDown(KEY_LEFT_CONTROL)) {
	...
	if (IsMouseButtonReleased(2) && isBoxSelecting) {
...
void ParticleSelection3D::boxSelection(UpdateParameters& myParam) {
	...
	if (!IsKeyDown(KEY_LEFT_CONTROL)) {
		if (IsMouseButtonDown(2) || (IsKeyDown(KEY_LEFT_ALT) && IsMouseButtonDown(2))) {
			if (IsMouseButtonPressed(2)) {
